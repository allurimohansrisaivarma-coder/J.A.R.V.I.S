...
class MemoryExtractor:
    ...
    async def extract_from_messages(self, messages: list[Message]) -> None:
        ...
        prompt = [Message.system(EXTRACTION_PROMPT), Message.user(f"Transcript:\n{transcript}")]

        try:
            # Generate extraction using the router
            router = self.session.router
            if router is None:
                logger.warning("Memory extraction skipped because the router is unavailable")
                return
            response = await router.generate_with_fallback(prompt)
                ...
                for fact in facts:
                    ...
                    if fact and not looks_like_artifact:
                        await asyncio.to_thread(
                            ...
                            fact,
                            source_context="Conversation transcript",
                        )
