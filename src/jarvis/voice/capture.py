...
class AudioCapture:
    ...
    async def listen_for_speech(self, silence_duration: float = 1.5, on_recording_start=None, on_audio_level=None) -> Path | None:
        ...
        ptt_buffer = collections.deque(maxlen=int(30 * self.sample_rate / self.chunk_size))
        ...
        try:
            ...
            logger.debug("Saved audio utterance", path=temp_path, duration=len(full_audio)/self.sample_rate)
            return Path(temp_path)
        except Exception as e:
            logger.error("Failed to save audio", error=str(e))
            return None

    async def record_until_released(self, event_queue: asyncio.Queue, on_audio_level=None) -> Path | None:
        ...
        try:
            sf.write(temp_path, full_audio, self.sample_rate)
            logger.debug("Saved PTT audio", path=temp_path, duration=len(full_audio)/self.sample_rate)
            return Path(temp_path)
        except Exception as e:
